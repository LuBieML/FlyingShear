...
def main(page: ft.Page):
    ...
    ROTARY_REFERENCE_CUTTING_RADIUS_PX = (
        ROTARY_DRUM_RADIUS_PX
        + ROTARY_KNIFE_LENGTH_PX
        + ROTARY_ACTIVE_KNIFE_EXTENSION_PX
    )
    ROTARY_CONTACT_GAP_PX = 2
    ...
    def rotary_visual_scale(geometry=None):
        geometry = geometry or rotary_sim_geometry()
        try:
            cutting_radius = compute_rotary_cutting_radius_px(
                geometry["drum_diameter_mm"],
                scale_px_per_unit[0],
                rotary_setting_float("link_units_to_mm", 1.0),
            )
        except ValueError:
            cutting_radius = ROTARY_REFERENCE_CUTTING_RADIUS_PX
        return max(0.05, cutting_radius / ROTARY_REFERENCE_CUTTING_RADIUS_PX)

    def rotary_scaled_visual_metrics(geometry=None):
        scale = rotary_visual_scale(geometry)
        contact_gap = max(1.0, ROTARY_CONTACT_GAP_PX * scale)
        return {
            "scale": scale,
            "drum_radius": ROTARY_DRUM_RADIUS_PX * scale,
            "knife_length": ROTARY_KNIFE_LENGTH_PX * scale,
            "active_extension": ROTARY_ACTIVE_KNIFE_EXTENSION_PX * scale,
            "contact_gap": contact_gap,
        }

    def rotary_required_sim_height():
        metrics = rotary_scaled_visual_metrics()
        top_clearance = 46.0 * metrics["scale"]
        guide_gap = (
            metrics["knife_length"]
            + metrics["active_extension"]
            + metrics["contact_gap"]
        )
        required = top_clearance + 2.0 * metrics["drum_radius"] + guide_gap + 86.0
        return max(ROTARY_SIM_HEIGHT, math.ceil(required))
    ...
    def rotary_draw_conveyor(shapes, width, height, belt_offset_px):
        rail_color = "#4a4f55"
    ...
    def draw_rotary_drum(shapes, width, belt_y, drum_angle):
        ...
        metrics = rotary_scaled_visual_metrics(geometry)
        drum_scale = metrics["scale"]
        r = metrics["drum_radius"]
        knife_length = metrics["knife_length"]
        active_extension_px = metrics["active_extension"]
        contact_gap = metrics["contact_gap"]

        def sc(value, minimum=0.0):
            return max(minimum, value * drum_scale)

        def sw(value):
            return max(0.45, value * drum_scale)

        guide_gap = knife_length + active_extension_px + contact_gap
        horizontal_margin = r + knife_length + active_extension_px + sc(42, 8)
        if width >= horizontal_margin * 2:
            cx = max(horizontal_margin, min(width - horizontal_margin, width * 0.32))
        else:
            cx = width / 2
        cy = belt_y - guide_gap - r
        ...
        guideline_paint = canvas_paint(
            ft.Colors.with_opacity(0.55, "#8fa1b3"),
            sw(1.2),
            dash=[sc(6, 1), sc(7, 1)],
        )
        shapes.append(cv.Line(cx, cy - r - sc(26), cx, belt_y + BELT_HEIGHT + sc(10), paint=guideline_paint))

        bed_y = belt_y - sc(12)
        block_w = sc(82, 4)
        block_h = sc(72, 4)
        left_block_x = max(sc(12), cx - r - sc(112))
        right_block_x = min(width - block_w - sc(12), cx + r + sc(30))
        block_y = cy - block_h / 2
        shaft_left = left_block_x + block_w - sc(8)
        shaft_right = right_block_x + sc(8)

        # Fixed machine hardware behind the rotating drum.
        shapes.extend([
            cv.Rect(cx - r - sc(86), bed_y - sc(6), 2 * (r + sc(86)), sc(8, 1),
                    border_radius=ft.BorderRadius.all(sc(3, 0.5)),
                    paint=linear_paint(cx, bed_y - sc(6), cx, bed_y + sc(2),
                                       ["#5f6871", "#262d33", "#101418"])),
            cv.Rect(left_block_x + sc(14), block_y + block_h - sc(2),
                    sc(16, 1), max(sc(18, 1), bed_y - block_y - block_h + sc(3)),
                    border_radius=ft.BorderRadius.all(sc(2, 0.5)),
                    paint=linear_paint(left_block_x, block_y, left_block_x, bed_y,
                                       ["#68727b", "#303840", "#13171b"])),
            cv.Rect(right_block_x + block_w - sc(30), block_y + block_h - sc(2),
                    sc(16, 1), max(sc(18, 1), bed_y - block_y - block_h + sc(3)),
                    border_radius=ft.BorderRadius.all(sc(2, 0.5)),
                    paint=linear_paint(right_block_x, block_y, right_block_x, bed_y,
                                       ["#68727b", "#303840", "#13171b"])),
            cv.Rect(left_block_x, block_y, block_w, block_h,
                    border_radius=ft.BorderRadius.all(sc(7, 0.5)),
            ...
            cv.Rect(right_block_x, block_y, block_w, block_h,
                    ...
                    paint=linear_paint(right_block_x, block_y, right_block_x, block_y + block_h,
                                       ["#74808a", "#303942", "#151a1f"])),
            cv.Rect(shaft_left, cy - sc(7), max(0, shaft_right - shaft_left), sc(14, 1),
                    border_radius=ft.BorderRadius.all(sc(7, 0.5)),
                    paint=linear_paint(cx, cy - sc(7), cx, cy + sc(7),
                                       ["#b7c0c7", "#69737c", "#20272d"],
                                       [0.0, 0.46, 1.0])),
            cv.Circle(left_block_x + block_w / 2, cy, sc(23, 0.5),
                      paint=fill_paint("#151b20")),
            cv.Circle(left_block_x + block_w / 2, cy, sc(15, 0.5),
                      paint=fill_paint("#56616b")),
            cv.Circle(right_block_x + block_w / 2, cy, sc(23, 0.5),
                      paint=fill_paint("#151b20")),
            cv.Circle(right_block_x + block_w / 2, cy, sc(15, 0.5),
                      paint=fill_paint("#56616b")),
        ])

        for bolt_x in (
            left_block_x + sc(13),
            left_block_x + block_w - sc(13),
            right_block_x + sc(13),
            right_block_x + block_w - sc(13),
        ):
            for bolt_y in (block_y + sc(13), block_y + block_h - sc(13)):
                shapes.append(cv.Circle(bolt_x, bolt_y, sc(3.3, 0.4), paint=fill_paint("#111418")))
                shapes.append(cv.Circle(bolt_x - sc(0.9), bolt_y - sc(0.9), sc(1.0, 0.3), paint=fill_paint("#aab2b9")))

        cut_window_color = SUCCESS_COLOR if in_cut else WARNING_COLOR if approaching else "#ccd24a"
        shapes.extend([
            cv.Arc(cx - r - sc(18), cy - r - sc(18), 2 * (r + sc(18)), 2 * (r + sc(18)),
                   start_angle=math.pi / 2 - half_window,
                   sweep_angle=2 * half_window,
                   paint=canvas_paint(ft.Colors.with_opacity(0.42, cut_window_color), sw(14))),
            cv.Arc(cx - r - sc(34), cy - r - sc(34), 2 * (r + sc(34)), 2 * (r + sc(34)),
                   start_angle=math.pi + math.radians(12),
                   sweep_angle=math.pi - math.radians(24),
                   paint=canvas_paint(ft.Colors.with_opacity(0.20, "#4fc3f7"), sw(11))),
            cv.Arc(cx - r - sc(34), cy - r - sc(34), 2 * (r + sc(34)), 2 * (r + sc(34)),
                   start_angle=math.pi + math.radians(12),
                   sweep_angle=math.pi - math.radians(24),
                   paint=canvas_paint(ft.Colors.with_opacity(0.58, "#8fb6c7"), sw(1.4), dash=[sc(12, 1), sc(7, 1)])),
            cv.Oval(cx - r - sc(22), cy + r - sc(14), 2 * (r + sc(22)), sc(34, 1),
        ...
        for idx in range(marker_count):
            ...
            mx, my = point_for(ROTARY_MATERIAL_CONTACT_ANGLE_RAD + offset, r + knife_length + contact_gap)
            shapes.append(cv.Circle(mx, my, sc(2.2, 0.35), paint=fill_paint(ft.Colors.with_opacity(0.72, cut_window_color))))

        shapes.extend([
            cv.Circle(cx, cy, r + sc(4),
            ...
            cv.Circle(cx, cy, max(1.0, r - sc(8)),
                      paint=ft.Paint(
            ...
            cv.Circle(cx, cy, r,
                      paint=canvas_paint("#07090c", sw(2.2))),
            cv.Circle(cx, cy, max(1.0, r - sc(23)),
                      paint=canvas_paint(ft.Colors.with_opacity(0.34, ft.Colors.WHITE), sw(0.9))),
        ])

        for slot_idx in range(24):
            theta = drum_angle + slot_idx * 2.0 * math.pi / 24.0
            sx0, sy0 = point_for(theta, max(0.0, r - sc(24)))
            sx1, sy1 = point_for(theta, max(0.0, r - sc(9)))
            line_color = ft.Colors.with_opacity(0.28 if slot_idx % 2 == 0 else 0.14, ft.Colors.WHITE)
            shapes.append(cv.Line(sx0, sy0, sx1, sy1, paint=canvas_paint(line_color, sw(1.0))))

        for spoke_idx in range(6):
            theta = drum_angle + spoke_idx * 2.0 * math.pi / 6.0
            oriented_box(theta, sc(36), max(sc(36), r - sc(34)), sc(15, 1), ft.Colors.with_opacity(0.26, "#0d1116"),
                         stroke=ft.Colors.with_opacity(0.16, ft.Colors.WHITE), stroke_width=sw(0.8))

        for bolt_idx in range(12):
            theta = drum_angle + bolt_idx * 2.0 * math.pi / 12.0
            bx, by = point_for(theta, sc(74))
            shapes.append(cv.Circle(bx, by, sc(4.6, 0.4), paint=fill_paint("#12161a")))
            shapes.append(cv.Circle(bx - sc(1.2), by - sc(1.2), sc(1.3, 0.3), paint=fill_paint("#b9c0c6")))

        shapes.extend([
            cv.Circle(cx, cy, sc(33, 0.6),
                      paint=ft.Paint(
                          gradient=ft.PaintRadialGradient(
                              center=ft.Offset(cx - sc(11), cy - sc(13)),
                              radius=sc(46, 1),
            ...
            cv.Circle(cx, cy, sc(33, 0.6), paint=canvas_paint("#090b0e", sw(1.2))),
            cv.Circle(cx, cy, sc(13, 0.5), paint=fill_paint("#161c22")),
            cv.Circle(cx, cy, sc(5, 0.35), paint=fill_paint("#050607")),
        ])
        ...
        for i, theta in blade_order:
            ...
            active_extension = active_extension_px if is_hot else 0
            blade_tip_radius = r + knife_length + active_extension

            oriented_box(theta, r - sc(17), r + sc(8), sc(44, 1), "#46515a", "#11161a", sw(1.0))
            oriented_box(theta, r - sc(8), r + sc(5), sc(34, 1), "#85909a", "#263039", sw(0.8))

            base_radius = r + sc(4)
            shoulder_radius = r + knife_length * 0.72
            tip_radius = blade_tip_radius
            root_w = sc(31 + (4 if is_hot else 0), 0.8)
            shoulder_w = sc(17 + (3 if is_hot else 0), 0.6)
            tip_w = sc(4, 0.35)
            ...
            tip_l = (cx + (tip_radius - sc(3)) * dx - tx * tip_w / 2, cy + (tip_radius - sc(3)) * dy - ty * tip_w / 2)
            tip = (cx + tip_radius * dx, cy + tip_radius * dy)
            tip_r = (cx + (tip_radius - sc(3)) * dx + tx * tip_w / 2, cy + (tip_radius - sc(3)) * dy + ty * tip_w / 2)
            ...
            add_polygon([root_l, shoulder_l, tip_l, tip, tip_r, shoulder_r, root_r],
                        blade_fill, blade_stroke, sw(1.25 if is_near else 1.0))

            bevel_mid = (cx + (base_radius + sc(12)) * dx - tx * sc(5), cy + (base_radius + sc(12)) * dy - ty * sc(5))
            bevel_tip = (cx + (tip_radius - sc(5)) * dx - tx * sc(1.5), cy + (tip_radius - sc(5)) * dy - ty * sc(1.5))
            shapes.append(cv.Line(bevel_mid[0], bevel_mid[1], bevel_tip[0], bevel_tip[1],
                                  paint=canvas_paint(ft.Colors.with_opacity(0.72, ft.Colors.WHITE), sw(1.1))))
            shapes.append(cv.Line(tip[0], tip[1], shoulder_r[0], shoulder_r[1],
                                  paint=canvas_paint("#7d858c", sw(0.9))))

            for bolt_offset in (-sc(12), sc(12)):
                bolt_x = cx + (r - sc(3)) * dx + tx * bolt_offset
                bolt_y = cy + (r - sc(3)) * dy + ty * bolt_offset
                shapes.append(cv.Circle(bolt_x, bolt_y, sc(2.6, 0.35), paint=fill_paint("#111418")))
                shapes.append(cv.Circle(bolt_x - sc(0.7), bolt_y - sc(0.7), sc(0.8, 0.25), paint=fill_paint("#d6dde2")))

            if is_hot:
                shapes.extend([
                    cv.Oval(tip[0] - sc(39), tip[1] - sc(10), sc(78, 1), sc(20, 1),
                            paint=fill_paint(ft.Colors.with_opacity(0.30, "#ffd54f"))),
                    cv.Line(tip[0] - sc(18), belt_y - contact_gap, tip[0] - sc(5), belt_y + sc(8),
                            paint=canvas_paint("#ffecb3", sw(1.2))),
                    cv.Line(tip[0] + sc(12), belt_y - sc(3), tip[0] + sc(25), belt_y + sc(7),
                            paint=canvas_paint("#ffb74d", sw(1.1))),
                ])
            elif is_near:
                shapes.append(cv.Circle(tip[0], tip[1], sc(8, 0.6),
                                        paint=fill_paint(ft.Colors.with_opacity(0.18, "#ffcc80"))))

        anvil_y = belt_y - sc(7)
        shapes.extend([
            cv.Rect(cx - sc(62), anvil_y, sc(124, 1), sc(7, 1),
                    border_radius=ft.BorderRadius.all(sc(2, 0.5)),
                    paint=linear_paint(cx, anvil_y, cx, anvil_y + sc(7),
                                       ["#c5ced5", "#68737d", "#222a31"])),
            cv.Rect(cx - sc(50), anvil_y - sc(4), sc(100, 1), sc(3, 1),
                    border_radius=ft.BorderRadius.all(sc(1.5, 0.5)),
    ...
    def redraw_rotary_sim():
        ...
        height = rotary_required_sim_height()
        rotary_sim_canvas.height = height
        rotary_sim_canvas_holder.height = height
        shapes = []
        belt_y = rotary_draw_conveyor(shapes, width, height, rotary_sim_state["belt_offset_px"])
    ...
    rotarylink_tooltips = {
        ...
        "repeat_step": "Legacy setting retained for compatibility; generated ROTARYLINK now increments start_pos by cut_length.",
        "buffered_commands": "Legacy setting retained for compatibility with older buffered examples.",
    ...
    rotarylink_sync_pos_input = make_input(
        "Start pos", "sync_pos", rotarylink_settings.get("sync_pos", rotarylink_legacy_base_acc), width=155
    )
    rotarylink_sync_pos_input.value = str(rotarylink_settings.get("sync_pos", rotarylink_legacy_base_acc))
    rotarylink_sync_pos_input.tooltip = rotarylink_tooltips["sync_pos"]
    rotarylink_sync_pos_input.read_only = True
    rotarylink_sync_pos_input.bgcolor = PANEL_ALT_BG
    ...
    rotarylink_repeat_step_input.disabled = True
    ...
    rotarylink_buffered_commands_input.disabled = True
    ...
    rotarylink_repeat_dropdown = make_dropdown(
        "Program style", "repeat_mode", rotarylink_settings.get("repeat_mode", "program_loop"),
        [
            ("program_loop", "WHILE loop"),
        ],
        width=205,
    )
    rotarylink_repeat_dropdown.value = "program_loop"
    ...
    rotarylink_merge_checkbox = ft.Checkbox(
        ...
        value=bool(rotarylink_settings.get("merge", False)),
        fill_color=ft.Colors.BLUE_700,
        check_color=ft.Colors.WHITE,
    ...
    rotarylink_calc_geometry_btn = ft.OutlinedButton(
        ...
        height=38,
        tooltip=(
            "Recomputes derived distance and saves UNITS = encoder "
    ...
    def rotarylink_build_phase_shapes(profile):
        ...
        if total <= 0:
            ...
            if minimum_total >= plot_width:
                widths = [
                    plot_width * value / total if value > 0 else 0
                ...
                widths = [
                    minimums[i] + (
                        remaining * phase_values[i] / total if phase_values[i] > 0 else 0
                    )
    ...
    def rotarylink_recalc(e=None):
        ...
        rotarylink_merge = False
        rotarylink_effective_merge = False
        rotarylink_uses_sync_pos = True
        ...
        rotarylink_include_optional = True
        ...
        try:
            ...
            rotarylink_profile_diag = {
                ...
                "cut_length": max(rotarylink_cut_length, 0.0),
                "link_dist": max(rotarylink_link_dist, 0.0),
                "linkdist": max(rotarylink_link_dist, 0.0),
            ...
            rotarylink_link_dist = rotarylink_profile_diag["link_dist"]
        ...
        rotarylink_merge_checkbox.value = rotarylink_required_merge
        rotarylink_options = build_rotarylink_options(
            rotarylink_start_mode,
            rotarylink_profile,
            rotarylink_link_source,
        ...
        if rotarylink_errors:
            ...
            if rotarylink_status["severity"] == "error":
                ...
                rotarylink_severity = "error"
            elif rotarylink_status["severity"] == "warning":
                rotarylink_messages.extend(rotarylink_status["messages"])
                rotarylink_severity = "warning"
            elif rotarylink_status["messages"]:
                rotarylink_messages.extend(rotarylink_status["messages"])
    ...
    def rotarylink_on_start_mode_change(e):
        rotarylink_current_repeat = rotarylink_repeat_dropdown.value or "program_loop"
        rotarylink_current_merge = bool(rotarylink_merge_checkbox.value)
        rotarylink_apply_sync_pos_state(
            rotarylink_start_dropdown.value or "absolute",
    ...
    def flexlink_profile_snapshot(master_delta=None):
        ...
        if master_delta is not None:
            flexlink_master_position = float(master_delta)
            flexlink_zero_tolerance = max(0.25, flexlink_cycle_pitch * 0.001)
            if abs(flexlink_master_position) <= flexlink_zero_tolerance:
                flexlink_phase = 0.0
            else:
                flexlink_cycles = math.floor(flexlink_master_position / flexlink_cycle_pitch)
                flexlink_phase = flexlink_master_position - flexlink_cycles * flexlink_cycle_pitch
                if flexlink_phase <= flexlink_zero_tolerance:
                    flexlink_phase = 0.0
                elif flexlink_cycle_pitch - flexlink_phase <= flexlink_zero_tolerance:
                    flexlink_cycles += 1
                    flexlink_phase = 0.0
            flexlink_u = max(0.0, min(1.0, flexlink_phase / flexlink_cycle_pitch))
    ...
    def flexlink_phase_from_slave_position(slave_position):
        ...
        flexlink_zero_tolerance = max(0.25, abs(flexlink_cycle_slave_dist) * 0.001)
        if abs(flexlink_slave_position) <= flexlink_zero_tolerance:
            flexlink_phase_position = 0.0
        else:
            flexlink_phase_position = flexlink_slave_position % flexlink_cycle_slave_dist
            if flexlink_phase_position <= flexlink_zero_tolerance:
                flexlink_phase_position = 0.0
            elif flexlink_cycle_slave_dist - flexlink_phase_position <= flexlink_zero_tolerance:
                flexlink_phase_position = 0.0
    ...
    def flexlink_sim_visual_width():
        return max(560, min(920, int(_available_visual_width() * 0.64)))

    def flexlink_metric_snapshot():
        flexlink_cycle_pitch = max(1.0, flexlink_setting_float("cycle_pitch", 180))
        flexlink_line_speed = max(0.0, flexlink_setting_float("line_speed", 400))
        flexlink_base_in_mm = max(0.0, flexlink_setting_float("base_in_mm", 108))
        ...
        return {
            "cycle_pitch": flexlink_cycle_pitch,
            "line_speed": flexlink_line_speed,
            "base_in_mm": flexlink_base_in_mm,
            "base_out_mm": flexlink_base_out_mm,
            "excite_acc_mm": flexlink_excite_acc_mm,
    ...
    def flexlink_draw_vertical_film_tube(flexlink_shapes, flexlink_width, flexlink_height,
        ...
        flexlink_tube_bottom = flexlink_height - 128.0
        flexlink_tube_h = flexlink_tube_bottom - flexlink_tube_top
        flexlink_center_x = flexlink_tube_x + flexlink_tube_w / 2.0
        flexlink_stroke_top = flexlink_tube_top + 52.0
        flexlink_stroke_bottom = flexlink_tube_bottom - 54.0
        ...
        flexlink_shapes.extend([
            cv.Rect(0, 0, flexlink_width, flexlink_height,
                    paint=ft.Paint(color="#070b0f", style=ft.PaintingStyle.FILL)),
            cv.Rect(18, 18, flexlink_width - 36, flexlink_height - 36,
                    border_radius=ft.BorderRadius.all(6),
                    paint=ft.Paint(color="#0b1015", style=ft.PaintingStyle.FILL)),
            cv.Rect(
                flexlink_center_x - 250,
                66,
                500,
                flexlink_tube_bottom - 36,
                border_radius=ft.BorderRadius.all(6),
                paint=ft.Paint(color=ft.Colors.with_opacity(0.34, "#111a20"), style=ft.PaintingStyle.FILL),
            ),
        ])

        collar_top = 48.0
        collar_bottom = 98.0
        collar_w = flexlink_tube_w + 132.0
        collar_points = [
            (flexlink_center_x - collar_w / 2.0, collar_top),
            (flexlink_center_x + collar_w / 2.0, collar_top),
            (flexlink_tube_x + flexlink_tube_w + 24.0, collar_bottom),
            (flexlink_tube_x - 24.0, collar_bottom),
        ]
        ...
        flexlink_shapes.append(
            cv.Line(
                ...
                collar_bottom,
                flexlink_center_x,
                flexlink_tube_bottom,
                paint=canvas_paint(ft.Colors.with_opacity(0.22, "#d4eef5"), 1.0, dash=[8, 10]),
            )
        )
        add_profile_text(
            flexlink_shapes, flexlink_center_x, collar_top + 14, "FORMING COLLAR",
            size=9, color=ft.Colors.GREY_300, max_width=150,
        )

        tube_paint = ft.Paint(
            gradient=ft.PaintLinearGradient(
                begin=ft.Offset(flexlink_tube_x, flexlink_tube_top),
                end=ft.Offset(flexlink_tube_x + flexlink_tube_w, flexlink_tube_top),
                colors=[
                    ft.Colors.with_opacity(0.18, "#dff9ff"),
                    ft.Colors.with_opacity(0.62, "#9fcbd4"),
                    ft.Colors.with_opacity(0.30, "#385762"),
                    ft.Colors.with_opacity(0.72, "#d9f2f7"),
                ],
                color_stops=[0.0, 0.32, 0.70, 1.0],
            ),
            style=ft.PaintingStyle.FILL,
        )
        flexlink_shapes.append(
        ...
        flexlink_shapes.extend([
            cv.Line(
                flexlink_tube_x + flexlink_tube_w * 0.20,
                flexlink_tube_top + 10,
                flexlink_tube_x + flexlink_tube_w * 0.20,
                flexlink_tube_bottom - 10,
                paint=canvas_paint(ft.Colors.with_opacity(0.38, ft.Colors.WHITE), 0.8),
            ),
            cv.Line(
                flexlink_tube_x + flexlink_tube_w * 0.74,
                flexlink_tube_top + 12,
                flexlink_tube_x + flexlink_tube_w * 0.74,
                flexlink_tube_bottom - 12,
        ...
        while flexlink_y < flexlink_tube_bottom + flexlink_bag_pitch_px:
            ...
            if flexlink_tube_top + 12 <= mark_y <= flexlink_tube_bottom - 12:
                flexlink_shapes.append(
                    cv.Rect(
                        flexlink_tube_x + flexlink_tube_w * 0.66,
                        mark_y - 5,
                        flexlink_tube_w * 0.18,
                        10,
                        border_radius=ft.BorderRadius.all(3),
                        paint=ft.Paint(color=ft.Colors.with_opacity(0.52, "#10181d"), style=ft.PaintingStyle.FILL),
                    )
                )
        ...
        return {
            ...
            "tube_w": flexlink_tube_w,
            "tube_top": flexlink_tube_top,
            "tube_bottom": flexlink_tube_bottom,
    ...
    def flexlink_draw_phase_strip(flexlink_shapes, flexlink_width, flexlink_height,
                                  flexlink_u, flexlink_base_in_mm, flexlink_base_out_mm,
                                  flexlink_open_window):
        strip_x = 54.0
        strip_y = flexlink_height - 72.0
        strip_w = flexlink_width - 108.0
        strip_h = 20.0
        total = max(1.0, flexlink_base_in_mm + flexlink_open_window + flexlink_base_out_mm)
        segments = [
            (flexlink_base_in_mm, "#146c7d", "SYNC"),
            (flexlink_open_window, "#95551f", "OPEN REG"),
            (flexlink_base_out_mm, "#146c7d", "SYNC"),
        ]
        cursor_x = strip_x + max(0.0, min(1.0, flexlink_u)) * strip_w
        flexlink_shapes.append(
            cv.Rect(strip_x, strip_y, strip_w, strip_h,
                    border_radius=ft.BorderRadius.all(4),
                    paint=ft.Paint(color="#111920", style=ft.PaintingStyle.FILL))
        )
        seg_x = strip_x
        for value, color, label in segments:
            seg_w = strip_w * max(0.0, value) / total
            if seg_w > 0.5:
                flexlink_shapes.append(
                    cv.Rect(seg_x, strip_y, seg_w, strip_h,
                            border_radius=ft.BorderRadius.all(4),
                            paint=ft.Paint(color=ft.Colors.with_opacity(0.78, color), style=ft.PaintingStyle.FILL))
                )
                if seg_w > 86:
                    add_profile_text(
                        flexlink_shapes, seg_x + seg_w / 2.0, strip_y + 5,
                        label, size=8, color=ft.Colors.WHITE, max_width=seg_w - 8,
                    )
            seg_x += seg_w
        flexlink_shapes.append(
            cv.Line(cursor_x, strip_y - 7, cursor_x, strip_y + strip_h + 8,
                    paint=canvas_paint("#f2f5a8", 1.8))
        )
        flexlink_shapes.append(
            cv.Circle(cursor_x, strip_y - 8, 4.0,
                      paint=ft.Paint(color="#f2f5a8", style=ft.PaintingStyle.FILL))
        )

    def flexlink_draw_jaw_pair(flexlink_shapes, flexlink_center_x, flexlink_y,
                               flexlink_tube_w, flexlink_closed, flexlink_label):
        flexlink_gap = 6.0 if flexlink_closed else 42.0
        jaw_w = 108.0
        jaw_h = 34.0
        top_y = flexlink_y - jaw_h / 2.0
        left_x = flexlink_center_x - flexlink_tube_w / 2.0 - flexlink_gap - jaw_w
        right_x = flexlink_center_x + flexlink_tube_w / 2.0 + flexlink_gap
        face_color = "#67e090" if flexlink_closed else "#e59a4a"
        shell_hi = "#cfd9dd" if flexlink_closed else "#b8c4ca"
        shell_mid = "#7f9098" if flexlink_closed else "#6f7d85"
        shell_lo = "#38464d" if flexlink_closed else "#323d44"
        carrier_color = "#172229"
        state_bg = "#10291b" if flexlink_closed else "#2f1c0d"

        flexlink_shapes.extend([
            cv.Line(left_x - 78, flexlink_y - 28, right_x + jaw_w + 78, flexlink_y - 28,
                    paint=canvas_paint("#31414a", 2.0)),
            cv.Line(left_x - 78, flexlink_y + 28, right_x + jaw_w + 78, flexlink_y + 28,
                    paint=canvas_paint("#223039", 1.6)),
            cv.Rect(left_x - 88, top_y + 3, 68, jaw_h - 6,
                    border_radius=ft.BorderRadius.all(4),
                    paint=ft.Paint(color=carrier_color, style=ft.PaintingStyle.FILL)),
            cv.Rect(right_x + jaw_w + 20, top_y + 3, 68, jaw_h - 6,
                    border_radius=ft.BorderRadius.all(4),
                    paint=ft.Paint(color=carrier_color, style=ft.PaintingStyle.FILL)),
        ])
        ...
        if flexlink_closed:
            flexlink_shapes.append(
                cv.Oval(
                    flexlink_center_x - flexlink_tube_w / 2.0 - 28,
                    flexlink_y - 39,
                    flexlink_tube_w + 56,
                    78,
                    paint=ft.Paint(color=ft.Colors.with_opacity(0.13, "#67e090"), style=ft.PaintingStyle.FILL),
                )
            )

        for jaw_x, side in ((left_x, "left"), (right_x, "right")):
            flexlink_shapes.append(
                cv.Rect(
                    jaw_x,
                    top_y,
                    ...
                    paint=ft.Paint(
                        gradient=ft.PaintLinearGradient(
                            begin=ft.Offset(jaw_x, top_y),
                            end=ft.Offset(jaw_x, top_y + jaw_h),
                            colors=[shell_hi, shell_mid, shell_lo],
                            color_stops=[0.0, 0.52, 1.0],
                        ),
                        style=ft.PaintingStyle.FILL,
                    ),
                )
            ...
            flexlink_shapes.append(
                cv.Line(jaw_x + 10, top_y + 7, jaw_x + jaw_w - 10, top_y + 7,
                        paint=canvas_paint(ft.Colors.with_opacity(0.35, ft.Colors.WHITE), 0.8))
            )
            if side == "left":
                face_x = jaw_x + jaw_w - 8
                flexlink_shapes.append(
                    cv.Rect(face_x, top_y + 4, 8, jaw_h - 8,
                            border_radius=ft.BorderRadius.all(2),
                            paint=ft.Paint(color=face_color, style=ft.PaintingStyle.FILL))
                )
            else:
                face_x = jaw_x
                flexlink_shapes.append(
        ...
        flexlink_shapes.append(
            cv.Rect(
                flexlink_center_x - 96,
                top_y - 35,
                192,
                22,
                border_radius=ft.BorderRadius.all(4),
                paint=ft.Paint(color=ft.Colors.with_opacity(0.88, state_bg), style=ft.PaintingStyle.FILL),
            )
        )
        add_profile_text(
            flexlink_shapes,
    ...
    def redraw_flexlink_sim():
        flexlink_width = float(flexlink_sim_canvas.width or flexlink_sim_visual_width())
        flexlink_height = float(flexlink_sim_canvas.height or flexlink_sim_height)
        flexlink_shapes = []
        flexlink_metrics = flexlink_metric_snapshot()
        flexlink_cycle_pitch = flexlink_metrics["cycle_pitch"]
        flexlink_base_in_mm = flexlink_metrics["base_in_mm"]
        ...
        flexlink_excite_progress, flexlink_in_excite = flexlink_excitation_progress(
            ...
            flexlink_base_in_mm,
            flexlink_base_out_mm,
            flexlink_metrics["excite_acc_mm"],
            flexlink_metrics["excite_dec_mm"],
            flexlink_curve_type,
        )
        ...
        rail_left = max(54.0, tube_x - 148.0)
        rail_right = min(flexlink_width - 54.0, tube_x + tube_w + 148.0)
        flexlink_shapes.extend([
            cv.Line(rail_left, stroke_top, rail_left, stroke_bottom, paint=canvas_paint("#4d606a", 2.0)),
            cv.Line(rail_right, stroke_top, rail_right, stroke_bottom, paint=canvas_paint("#4d606a", 2.0)),
            cv.Line(rail_left - 14, stroke_top, rail_right + 14, stroke_top,
                    paint=canvas_paint(ft.Colors.with_opacity(0.20, "#cfe6ee"), 1.0)),
            cv.Line(rail_left - 14, stroke_bottom, rail_right + 14, stroke_bottom,
                    paint=canvas_paint(ft.Colors.with_opacity(0.20, "#cfe6ee"), 1.0)),
        ])
        add_profile_text(
            flexlink_shapes,
        ...
        flexlink_shapes.append(
            cv.Line(center_x + tube_w / 2.0 + 64, tube_top + 10,
        ...
        flexlink_add_polygon(
            flexlink_shapes,
            [
                (center_x + tube_w / 2.0 + 58, tube_bottom - 34),
                (center_x + tube_w / 2.0 + 70, tube_bottom - 34),
                (center_x + tube_w / 2.0 + 64, tube_bottom - 20),
            ],
            "#8aa8b6",
            None,
            0,
        ...
        if flexlink_live:
            ...
            flexlink_shapes.append(
                cv.Rect(
                    ...
                    48,
                    border_radius=ft.BorderRadius.all(5),
                    paint=ft.Paint(color="#121a20", style=ft.PaintingStyle.FILL),
                )
            )
            add_profile_text(
                flexlink_shapes,
    ...
    flexlink_sim_canvas = cv.Canvas(
        width=flexlink_sim_visual_width(),
    ...
    flexlink_phase_state_label = ft.Text("--", size=15, color=ft.Colors.CYAN_200, weight=ft.FontWeight.BOLD)
    ...
    def flexlink_diag_card(label, value_control, width=185, col=None):
        ...
        card_width = None if col is not None else width
        return ft.Container(
            content=ft.Column([
                ft.Text(label, size=11, color=ft.Colors.GREY_400),
                value_control,
    ...
    def update_flexlink_diagnostics(now, force=False):
        ...
        flexlink_excite_progress, flexlink_in_excite = flexlink_excitation_progress(
            flexlink_u,
            flexlink_metrics["cycle_pitch"],
            flexlink_metrics["base_in_mm"],
            flexlink_metrics["base_out_mm"],
            flexlink_metrics["excite_acc_mm"],
            flexlink_metrics["excite_dec_mm"],
            str(flexlink_settings.get("curve_type", "sine")),
        )
        flexlink_in_seal_contact = not flexlink_in_excite
        try:
        ...
        if flexlink_live:
            ...
            flexlink_status_text.value = "Live MPOS/MSPEED from controller. Jaw position follows absolute slave MPOS."
            flexlink_status_text.color = SUCCESS_COLOR
        elif trio_conn.is_connected():
            flexlink_source_label.value = "Waiting"
            flexlink_source_label.color = WARNING_COLOR
            flexlink_status_text.value = "Connected; waiting for live MPOS/MSPEED monitor reads."
            flexlink_status_text.color = WARNING_COLOR
        else:
            flexlink_source_label.value = "Controller required"
            flexlink_source_label.color = WARNING_COLOR
            flexlink_status_text.value = "Connect to a controller to enable live simulation."
        ...
        flexlink_phase_label.value = f"{flexlink_u * 100.0:.1f}%" if flexlink_live else "--"
        flexlink_phase_state_label.value = (
            "1:1 seal downstroke" if flexlink_in_seal_contact else "Open return / correction"
        ) if flexlink_live else "No live phase"
        flexlink_phase_state_label.color = (
            SUCCESS_COLOR if flexlink_in_seal_contact else ft.Colors.ORANGE_200
        ) if flexlink_live else MUTED_TEXT
        ...
        if flexlink_live and flexlink_error is not None:
            ...
            flexlink_tolerance = max(0.02, flexlink_metrics["cycle_pitch"] * 0.005)
    ...
    def update_flexlink_sim_from_reads(master_mpos, slave_mpos, master_mspeed, slave_mspeed, now):
        ...
        if master_mpos is not None:
            ...
            flexlink_sim_state["belt_offset_px"] = (
                CONVEYOR_VISUAL_DIRECTION * master_mpos * scale_px_per_unit[0]
            ) % BELT_SPACING
            dirty = True
    ...
    flexlink_live_readings_row = ft.ResponsiveRow(
        [
            flexlink_diag_card("Source", flexlink_source_label, col={"xs": 12, "sm": 6}),
            flexlink_diag_card("Master speed", flexlink_master_speed_label, col={"xs": 12, "sm": 6}),
            flexlink_diag_card("Jaw speed", flexlink_slave_speed_label, col={"xs": 12, "sm": 6}),
            flexlink_diag_card("Cycle phase", flexlink_phase_label, col={"xs": 12, "sm": 6}),
            flexlink_diag_card("Position error", flexlink_sync_error_label, col={"xs": 12, "sm": 6}),
        ],
        columns=12,
        spacing=10,
        run_spacing=10,
    ...
    flexlink_cycle_readings_grid = ft.ResponsiveRow(
        [
            flexlink_diag_card("Phase", flexlink_phase_state_label, col={"xs": 12}),
            ...
            [
                ft.Row(
                    [
                        ...
                        ft.Text("SIMULATION STATUS", size=10, color=MUTED_TEXT, weight=ft.FontWeight.BOLD),
                    ],
                    spacing=6,
                    vertical_alignment=ft.CrossAxisAlignment.CENTER,
                ),
    ...
    flexlink_visual_panel = ft.Container(
        content=ft.Column(
            [
                ft.Row(
                    [
                        ft.Icon(ft.Icons.PRECISION_MANUFACTURING, size=14, color=MUTED_TEXT),
                        ft.Text("MACHINE CYCLE", size=10, color=MUTED_TEXT, weight=ft.FontWeight.BOLD),
                    ],
                    spacing=6,
                    vertical_alignment=ft.CrossAxisAlignment.CENTER,
                ),
    ...
    flexlink_right_panel = ft.Container(
        content=ft.Column(
            [
                ...
                ft.Row(
                    [
                        ft.Icon(ft.Icons.TIMELINE, size=14, color=MUTED_TEXT),
                        ft.Text("CYCLE STATE", size=10, color=MUTED_TEXT, weight=ft.FontWeight.BOLD),
                    ],
                    spacing=6,
                    vertical_alignment=ft.CrossAxisAlignment.CENTER,
                ),
                flexlink_cycle_readings_grid,
                ft.Row(
                    [
                        ft.Icon(ft.Icons.SPEED, size=14, color=MUTED_TEXT),
                        ft.Text("LIVE READINGS", size=10, color=MUTED_TEXT, weight=ft.FontWeight.BOLD),
                    ],
                    spacing=6,
                    vertical_alignment=ft.CrossAxisAlignment.CENTER,
                ),
    ...
    flexlink_sim_container = ft.Container(
        content=ft.Column(
            [
                section_header("VFFS Cross-Seal Simulation", "Live jaw carriage and film web phase", ft.Icons.PLAY_CIRCLE),
                ft.ResponsiveRow(
                    [flexlink_visual_panel, flexlink_right_panel],
                    columns=12,
                    spacing=18,
                    run_spacing=18,
                    vertical_alignment=ft.CrossAxisAlignment.START,
    ...
    def resize_flexlink_sim_visual(update=False):
        flexlink_new_visual_width = flexlink_sim_visual_width()
            ...
            ft.ResponsiveRow(
                [
                    ...
                    help_card(
                        "Sync Position",
                        [
                            help_text("The generated loop keeps a start_pos variable for the next ROTARYLINK call and increments it by cut_length after every command."),
                            formula_block([
                                "start_pos = base_acc",
                                "ROTARYLINK(..., moveoptions, start_pos)",
                                "start_pos = start_pos + cut_length",
                            ]),
                            help_text("There is no idle phase argument in ROTARYLINK; product spacing belongs in this surrounding loop."),
                        ],
                        icon=ft.Icons.PLACE,
                        col={"xs": 12, "xl": 6},
                    ),
    ...
    shell = AppShell(
        ctx,
        load_axis_param_controls_for_solution=load_axis_param_controls_for_solution,
        flexlink_start_sim=flexlink_start_sim,
        flexlink_stop_sim=flexlink_stop_sim,
        set_rotary_sim_labels=set_rotary_sim_labels,
        prompt_solution_axis_params_if_connected=prompt_solution_axis_params_if_connected,
        point_to_point_control_page=point_to_point_control_page,
        axis_connection_page=axis_connection_page,
        cam_calc_container=cam_calc_container,
        rotary_profile_container=rotary_profile_container,
        rotary_cam_math_help_list=rotary_cam_math_help_list,
        rotary_sim_container=rotary_sim_container,
        flexlink_calc_container=flexlink_calc_container,
        flexlink_sim_container=flexlink_sim_container,
        flexlink_help_list=flexlink_help_list,
        rotarylink_calc_container=rotarylink_calc_container,
        rotarylink_help_list=rotarylink_help_list,
        shear_calc_container=shear_calc_container,
        movelink_help_list=movelink_help_list,
        flying_shear_monitor_page=flying_shear_monitor_page,
    )
